...
contract BlackjackGame {
    ...
    constructor(BlackjackToken btAddress) public {
    ...
    * @return A uint8 representing the value of the input hand
    */
    ...
    */
    ...
    /**
    * @dev Function to start a new game of blackjack
    * @param betAmountInTokens A uint256 representing the number of BT tokens the player wants to bet
    */
    function startNewGame(uint256 betAmountInTokens) public {
        ...
        require(playerCredit >= betAmountInTokens, "Insufficient credits.");

        // Reset hands, bets, and set game state to true
        initializeDeck(msg.sender);
        shuffleDeck(msg.sender);
        betAmounts[msg.sender] = betAmountInTokens;
        totalBets[msg.sender] = betAmountInTokens;
        ...
        emit logInteger(getHandValue(playerHand));
        emit logEvent("Dealer's Hand");
        emit logInteger(getHandValue(dealerHand));
